...
all = (
    ...
    RS(0x206, (
        ...
        R('grid_current_b', 'Grid B current (0.01A)', signed=True),
        R('grid_voltage_c', 'Grid C voltage (0.1V)'),
        R('grid_current_c', 'Grid C current (0.01A)', signed=True),
        R('grid_frequency', 'Grid frequency (0.01Hz)'),
        R('battery_charge_power', 'Battery charge power (0.01KW)', signed=True),
        R('battery_voltage', 'Battery voltage (0.1V)'),
        R('battery_charge_current', 'Battery charge current (0.01A)', signed=True),
        R('battery_capacity_soc', 'Battery charge (%)'),
        R('battery_temperature', 'Battery temperature (0.1C)'),
        R('feed_in_power', 'Feed in power (0.01KW)', signed=True),
    ...
    RS(0x22A, (
        ...
        R('grid_current_r', 'Grid R current (0.01A)', signed=True),
        R('grid_voltage_s', 'Grid S voltage (0.1V)'),
        R('grid_current_s', 'Grid S current (0.01A)', signed=True),
        R('grid_voltage_t', 'Grid T voltage (0.1V)'),
        R('grid_current_t', 'Grid T current (0.01A)', signed=True),
        R('generation_current', 'Generation current (?)', signed=True),
        R('battery_power', 'Battery power (?)'),
        R('inverter_temperature', 'Inverter temperature (1C)', signed=True),
        R('heat_sink_temperature', 'Heat sink temperature (1C)', signed=True),
        R('country', 'Country'),
        R('current_dc_component', 'Current DC component(1mA)', signed=True),
